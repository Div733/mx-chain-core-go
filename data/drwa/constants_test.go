...
func TestDenialUnknown_IsNotStorable(t *testing.T) {
	if DenialUnknown.IsValid() {
		t.Fatalf("DenialUnknown must not be valid for storage — it is not attributable to any compliance rule")
	}
	if !DenialUnknown.IsKnown() == false {
		// IsKnown must also return false for DenialUnknown
	}
	if DenialUnknown.IsKnown() {
		t.Fatalf("DenialUnknown must not be a known concrete denial code")
	}
}
...
func TestAllDenialCodes_Complete(t *testing.T) {
	all := AllDenialCodes()
	if len(all) != 15 {
		t.Fatalf("expected 15 denial codes, got %d — update this test and the compliance gate switch in mx-chain-vm-common-go", len(all))
	}
